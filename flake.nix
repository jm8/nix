{
  ...
  outputs = {
    ...
    overlayFor = getStdenv: final: prev: let
    ...
    in {
      ...
      with commonDeps {inherit pkgs;};
        currentStdenv.mkDerivation {
          ...
          # nix-analyzer: the tests gives a segmentation fault ¯\_(ツ)_/¯
          doCheck = false;
          ...
          passthru.perl-bindings = with final;
            perl.pkgs.toPerlModule (currentStdenv.mkDerivation {
      ...
      lowdown-nix = with final;
        currentStdenv.mkDerivation rec {
          name = "lowdown-0.9.0";
          ...
          configurePhase = ''
            ${
              if (currentStdenv.isDarwin && currentStdenv.isAarch64)
              then "echo \"HAVE_SANDBOX_INIT=false\" > configure.local"
              else ""
            }
            ./configure \
  ...
  in {
    ...
    hydraJobs = {
      ...
      buildCross =
        nixpkgs.lib.genAttrs crossSystems (crossSystem:
      ...
      binaryTarballCross = nixpkgs.lib.genAttrs ["x86_64-linux"] (system:
        builtins.listToAttrs (map (crossSystem: {
            ...
            in
              binaryTarball nixpkgsFor.${system} self.packages.${system}."nix-${crossSystem}" nixpkgsCross;
          })
          crossSystems));
      ...
      coverage = with nixpkgsFor.x86_64-linux;
      with commonDeps {inherit pkgs;};
      ...
      tests.evalNixpkgs = with nixpkgsFor.x86_64-linux;
      ...
      installTests = forAllSystems (system: let
    ...
    packages = forAllSystems (system:
      ...
      // (nixpkgs.lib.optionalAttrs (builtins.elem system linux64BitSystems) {
        ...
        // builtins.listToAttrs (map (crossSystem: {
            ...
            in
              with commonDeps {pkgs = nixpkgsCross;};
                nixpkgsCross.stdenv.mkDerivation {
                  name = "nix-${version}";
